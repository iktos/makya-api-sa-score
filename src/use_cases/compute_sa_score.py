...
def __numBridgeheadsAndSpiro(mol, ri=None):
    if ri is None:
        ri = mol.GetRingInfo()

    nSpiro = __count_spiros(ri)
    nBridges = __count_bridges(mol, ri)

    return nBridges, nSpiro


def __count_spiros(ri):
...
def compute_sa_score(m):
    if _fscores is None:
        __readFragmentScores()
    ...
    ri = m.GetRingInfo()
    nBridgeheads, nSpiro = __numBridgeheadsAndSpiro(m, ri)
    nMacrocycles = 0
